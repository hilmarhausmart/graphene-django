...
def url_string(string="/rest_framework/graphql", **url_params):
    if url_params:
...
def batch_url_string(**url_params):
    return url_string("/rest_framework/graphql/batch", **url_params)
...
def test_batch_fails_if_is_empty(client):
    ...
    assert response.status_code == 400
    assert response_json(response) == {
        "errors": [{"message": "JSON parse error - Received an empty list in the batch request."}]
    }
...
def test_handles_not_expected_json_bodies(client):
    ...
    assert response.status_code == 400
    assert response_json(response) == {
        "errors": [{"message": "JSON parse error - The received data is not a valid JSON query."}]
    }
...
def test_handles_invalid_json_bodies(client):
    ...
    assert response.status_code == 400
    assert response_json(response) == {
        "errors": [{"message": "JSON parse error - Expecting value: line 1 column 2 (char 1)"}]
    }
...
def test_handles_plain_post_text(client):
    ...
    assert response_json(response) == {
        "errors": [{"message": 'Unsupported media type "text/plain" in request.'}]
    }
